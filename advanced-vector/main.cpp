...
void Test6() {
    ...
    }
    {
        ...
        Vector<Obj> v{SIZE};
        Obj obj{ 1 };
        Vector<Obj>::iterator pos = v.Insert(v.cbegin() + 1, std::move(obj));
        assert(v.Size() == SIZE + 1);
        assert(v.Capacity() == SIZE * 2);
        assert(&*pos == &v[1]);
        assert(v[1].id == obj.id);
        std::cerr << Obj::num_copied << std::endl;
      //  assert(Obj::num_copied == 1);
        assert(Obj::num_default_constructed == SIZE);
...
/*
    {
        size_t ID = 42, SIZE = 10;

        
        Obj::ResetCounters();
        Vector<Obj> v{SIZE};
        auto* pos = v.Emplace(v.cbegin() + 1, ID, "Ivan");
        assert(v.Size() == SIZE + 1);
        assert(v.Capacity() == SIZE * 2);
        assert(&*pos == &v[1]);
        assert(v[1].id == ID);
        assert(v[1].name == "Ivan");
        assert(Obj::num_copied == 0);
        assert(Obj::num_default_constructed == SIZE);
        assert(Obj::num_moved == SIZE);
        assert(Obj::num_move_assigned == 0);
    ...
    auto* pos = v.Emplace(v.cbegin() + 3, v[4]);
